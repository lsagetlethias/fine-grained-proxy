...
- **G4, query.** Les paramètres de query sont soit contraints par le scope quand celui-ci le déclare, soit transmis librement, et l'outillage dit lequel des deux s'applique. Il n'existe aucun cas où l'interface affirme une contrainte que le proxy n'applique pas. Au moment de cet ADR, `queryFilters` n'étant pas livré (voir §4), le seul état atteignable était « transmis librement », et c'était dit comme tel. **La feature a depuis été livrée** : les deux états sont désormais atteignables, et G4 est tenue dans sa forme complète.
...
> **Statut : livré.** Cette section décrit la décision et son report au moment où ils ont été pris, et son raisonnement reste la référence. L'implémentation a suivi depuis, en feature séparée comme prévu. Ce qui y est écrit au futur ou comme non encore fait est donc à lire comme de l'histoire : `docs/specs.md` section 19 décrit le comportement livré, et fait foi en cas d'écart. Les seuls points que la livraison a tranchés au-delà de cet ADR sont la restriction du type `any` aux chaînes, le plafond d'occurrences à deux paliers, et la règle de version en plancher par axe.
...
- **L'axe query restait ouvert jusqu'à la livraison de `queryFilters`.** Entre ce lot et la feature, un blob scopé sur un chemin autorisait toutes les querys de ce chemin, `?force=true` compris. Le lot de sécurité a rendu ce trou visible et a cessé de mentir dessus sans le fermer, arbitrage de calendrier assumé et non réévaluation du risque. **La feature a depuis été livrée et le ferme**, pour les scopes qui déclarent des `queryFilters` : un scope qui n'en déclare pas garde le comportement décrit ici, et c'est le cas de tous les blobs antérieurs.
