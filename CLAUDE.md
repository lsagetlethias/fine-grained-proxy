...
- **Base du worktree** : le harness crée le worktree depuis `main`, **pas depuis la branche courante**. Un agent lancé sur une branche de feature n'y est donc pas, et le symptôme est trompeur puisque son dépôt est cohérent, juste au mauvais endroit. Nommer le commit attendu dans le brief, les fiches d'agent portent la vérification. Et un worktree neuf ne passe pas `deno task check` sans un `deno task build` préalable, `changelog-data.ts` et `static/` étant générés.
